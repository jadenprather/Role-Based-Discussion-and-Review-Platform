...
public class PostServiceTest {
    ...
    public static void main(String[] args) throws Exception { new PostServiceTest().run(); }
    ...
    private void search_thread_query_deleted_flag(){
        ...
        var math = repo.search(new SearchSpec().thread("math"));
        assertEq(2, math.size(), "thread filter");

        var arraysDefault = repo.search(new SearchSpec().query("arrays"));
        assertEq(0, arraysDefault.size(), "deleted excluded");

        var arraysIncl = repo.search(new SearchSpec().query("arrays").includeDeleted(true));
